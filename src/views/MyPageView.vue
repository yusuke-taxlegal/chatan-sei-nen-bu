...
import { EmailAuthProvider, reauthenticateWithCredential, updatePassword, updateEmail, GoogleAuthProvider, linkWithPopup, deleteUser } from "firebase/auth";
...
const saveProfile = async () => {
  ...
  try {
    ...
    const profileData = {
      ...
      sns: sns.value,
      youtube: youtube.value, // 保存データにYouTubeを追加
...
const handleChangeEmail = async () => {
  ...
  try {
    ...
    emailChangeMessage.value = 'メールアドレスが正常に変更されました。新しいアドレスに送信されたメールをご確認の上、認証を完了してください。';
...
const handleDeleteAccount = async () => {
  ...
  try {
    ...
    if (error.code === 'auth/wrong-password') {
      ...
      deleteError.value = '最終ログインから時間が経過しています。安全のため、もう一度ログインしてからお試しください。';
    } else {
  ...
  <div class="page-container">
      ...
      <div class="profile-layout">
        <div class="image-section">
          <div class="image-preview-wrapper" @click="triggerFileInput">
            <img :src="profileImageUrl || 'https://via.placeholder.com/150'" alt="プロフィール画像" class="profile-image-preview"/>
            <div class="image-overlay"><span>📷 変更</span></div>
          </div>
          <button @click="triggerFileInput" class="image-select-button">画像を選択</button>
          <input type="file" ref="fileInputRef" @change="handleFileSelect" accept="image/*" class="file-input" />
        </div>
          ...
          <div class="form-grid">
            <div class="grid-col-2">
              ...
              <div class="form-group">
                ...
                <input type="text" id="phoneticName" v-model="phoneticName" placeholder="フリガナを入力" />
              </div>
            </div>
            <div class="form-group">
              <label for="companyName">🏢 会社名</label>
              <input type="text" id="companyName" v-model="companyName" placeholder="会社名を入力" />
            </div>
            <div class="form-group">
              <label for="bio">📝 自己紹介・事業内容</label>
              <textarea id="bio" v-model="bio" rows="5" placeholder="ご自身の事業や趣味、得意なことなどを自由に紹介してください"></textarea>
            </div>
            <div class="form-group">
              <label for="needs">🎯 どのようなニーズに対応できるか</label>
              <textarea id="needs" v-model="needs" rows="3" placeholder="例）会社の忘年会や接待で利用できる飲食店を探している方。"></textarea>
            </div>
            <div class="form-group">
              <label for="pastTransactions">🤝 過去に青年部内で取引した事例</label>
              <textarea id="pastTransactions" v-model="pastTransactions" rows="3" placeholder="例）○○さんのオフィスの内装工事を担当しました。"></textarea>
            </div>
            <div class="form-group">
              <label for="providableInfo">💡 提供できる情報</label>
              <textarea id="providableInfo" v-model="providableInfo" rows="3" placeholder="例）車両を長持ちさせるメンテナンスのコツ。"></textarea>
            </div>
            <div class="form-group">
              <label for="seekingInfo">🔍 求めている情報</label>
              <textarea id="seekingInfo" v-model="seekingInfo" rows="3" placeholder="例）新メニューのPRを手伝ってくれる事業者を探しています。"></textarea>
            </div>
            <div class="grid-col-2">
              <div class="form-group">
                <label for="website">🌐 ウェブサイト</label>
                <input type="url" id="website" v-model="website" placeholder="https://example.com" />
              </div>
              <div class="form-group">
                <label for="sns">📢 SNSリンク</label>
                <input type="url" id="sns" v-model="sns" placeholder="https://twitter.com/username" />
              </div>
            </div>
            <div class="form-group">
              <label for="youtube">📺 YouTubeリンク</label>
              <input type="url" id="youtube" v-model="youtube" placeholder="https://youtube.com/channel/..." />
            </div>
    ...
    <template v-if="authProvider === 'password'">
      <div class="card">
        ...
        <p class="card-subtitle">お使いのGoogleアカウントと連携すると、次回からGoogleボタンで簡単にログインできるようになります。</p>
        <button @click="linkWithGoogle" class="save-button google-button" :disabled="isLinkingGoogle">
          <div class="google-icon"><svg width="18" height="18" viewBox="0 0 18 18"><path fill="#4285F4" d="M16.51 8H8.98v3h4.3c-.18 1-.74 1.48-1.6 2.04v2.01h2.6a7.8 7.8 0 0 0 2.38-5.88c0-.57-.05-.66-.15-1.18z"></path><path fill="#34A853" d="M8.98 17c2.16 0 3.97-.72 5.3-1.94l-2.6-2.04a4.8 4.8 0 0 1-7.18-2.53H1.83v2.07A8 8 0 0 0 8.98 17z"></path><path fill="#FBBC05" d="M4.5 10.49a4.8 4.8 0 0 1 0-3.07V5.35H1.83a8 8 0 0 0 0 7.22l2.67-2.08z"></path><path fill="#EA4335" d="M8.98 4.72c1.16 0 2.23.4 3.06 1.2l2.3-2.3A8 8 0 0 0 1.83 5.35L4.5 7.42a4.77 4.77 0 0 1 4.48-2.7z"></path></svg></div>
      ...
      <div class="card">
        ...
        <div class="form-group">
          ...
          <input type="email" id="currentEmail" :value="currentEmail" readonly class="readonly-input" />
        </div>
        <div class="grid-col-2">
          <div class="form-group">
            <label for="newEmail">📧 新しいメールアドレス</label>
            <input type="email" id="newEmail" v-model="newEmail" placeholder="新しいメールアドレス" />
          </div>
          <div class="form-group">
            <label for="passwordForEmailChange">🔑 現在のパスワード</label>
            <input type="password" id="passwordForEmailChange" v-model="passwordForEmailChange" placeholder="本人確認のため" />
          </div>
        </div>
        <button @click="handleChangeEmail" class="save-button" :disabled="isEmailChanging">
          <span v-if="isEmailChanging" class="loader"></span><span v-else>メールアドレスを変更</span>
        </button>
      ...
      <div class="card">
        ...
        <div class="form-group">
          ...
          <input type="password" id="currentPassword" v-model="currentPassword" placeholder="現在のパスワード" />
        </div>
        <div class="grid-col-2">
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="password" id="confirmPassword" v-model="confirmPassword" placeholder="再度入力" />
          </div>
        </div>
        ...
        <div v-if="passwordChangeMessage" class="alert-message success">{{ passwordChangeMessage }}</div>
    ...
    <div v-if="authProvider === 'google.com'" class="card">
      ...
      <div class="info-message">
        <p class="info-text">このアカウントはGoogleアカウントと連携しています。<br>メールアドレスやパスワードの変更は、ご利用のGoogleアカウント設定から行ってください。</p>
      </div>
    </div>
    ...
    <div class="card delete-card">
      ...
      <p class="card-subtitle">この操作は元に戻すことはできません。アカウントを削除すると、すべてのプロフィール情報が完全に削除されます。</p>
      <button @click="showDeleteModal = true" class="save-button delete-button">アカウントを削除する</button>
    </div>
    ...
    <div v-if="showCropperModal" class="cropper-modal-overlay">
      <div class="cropper-modal-content">
        <h3>画像を調整</h3>
        <cropper ref="cropperRef" :src="imageToCrop" :stencil-component="CircleStencil" :stencil-props="{ aspectRatio: 1/1 }" />
    ...
    <div v-if="showDeleteModal" class="cropper-modal-overlay">
      <div class="cropper-modal-content delete-modal-content">
        ...
        <div v-if="authProvider === 'password'" class="form-group">
          ...
          <input type="password" id="passwordForDelete" v-model="passwordForDelete" placeholder="現在のパスワード" />
        </div>

        <!-- For Google users -->
        <div v-if="authProvider === 'google.com'" class="form-group">
          <label for="deleteConfirmation">「削除します」と入力してください</label>
          <input type="text" id="deleteConfirmation" v-model="deleteConfirmationText" placeholder="削除します" />
        </div>

        <div v-if="deleteError" class="alert-message error">{{ deleteError }}</div>

        <div class="cropper-actions">
          <button @click="showDeleteModal = false" class="cropper-button secondary">キャンセル</button>
          <button @click="handleDeleteAccount" class="cropper-button delete-button" :disabled="isDeleting">
...
.cropper-modal-content h3 { font-size: 1.5rem; margin-bottom: 1.5rem; }
.cropper-actions { display: flex; justify-content: flex-end; gap: 1rem; margin-top: 1.5rem; }
.cropper-button { padding: 0.6rem 1.2rem; border-radius: 0.5rem; border:none; cursor:pointer; }
.cropper-button.primary { background: var(--vt-c-brand); color:white; }
.cropper-button.secondary { background: var(--color-background); color: var(--color-text); border: 1px solid var(--color-border); }
